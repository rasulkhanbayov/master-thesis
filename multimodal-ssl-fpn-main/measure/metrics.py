...
class BCE(Metrics):
    ...
    def calculate_batch(self, ground: dict, predict: dict) -> np.ndarray:
        ...
        gr = gr[:, self.slice].reshape(gr.size(0), -1)
...
class FocalLoss(Metrics):
    def __init__(self, output_key: str, target_key: str, alpha=0.25, gamma=2.0, slice: int = 0):
        super().__init__()
        self.output_key = output_key
        self.target_key = target_key
        self.alpha = alpha
    ...
    def calculate_batch(self, ground: dict, predict: dict) -> np.ndarray:
        ...
        focal_loss = focal_loss.mean(dim=1).cpu().numpy()

        return focal_loss

class CrossEntropyLoss(Metrics):
    def __init__(self, output_key: str, target_key: str, slice: int = 0):
        super().__init__()
        self.output_key = output_key
        self.target_key = target_key
    ...
    def calculate_batch(self, ground: dict, predict: dict) -> np.ndarray:
        ...
        ce_loss = ce_loss.mean(dim=1).cpu().numpy()
